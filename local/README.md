...
Create `local/.env` from `.env.example`, set a private `LOCAL_INGEST_TOKEN`, and set `WORKER_STATUS_URL` to the Worker base URL. Start it with:

```bash
python3 local/server.py
```

Open the local service URL for the dashboard. The probes use the same token through `LOCAL_URL` and `LOCAL_INGEST_TOKEN`.
